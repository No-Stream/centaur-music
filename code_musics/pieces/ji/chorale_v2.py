...
from code_musics.composition import grid_line, with_synth_ramp
...
def build_ji_chorale_v2_score() -> Score:
    ...
    score.add_voice(
        ...
        effects=[
            EffectSpec(
                "eq",
                {"bands": [{"kind": "highpass", "cutoff_hz": 100.0, "slope_db_per_oct": 24}]},
            ),
        ],
        mix_db=-11.0,
        pan=-0.16,
    ...
    score.add_voice(
        ...
        effects=[
            EffectSpec(
                "eq",
                {"bands": [{"kind": "highpass", "cutoff_hz": 120.0, "slope_db_per_oct": 24}]},
            ),
        ],
        mix_db=-9.5,
        pan=0.14,
    ...
    score.add_voice(
        ...
        effects=[
            EffectSpec(
                "eq",
                {"bands": [{"kind": "highpass", "cutoff_hz": 180.0, "slope_db_per_oct": 24}]},
            ),
        ],
        mix_db=-10.0,
        pan=0.08,
    ...
    score.add_voice(
        ...
        effects=[
            EffectSpec(
                "eq",
                {"bands": [{"kind": "highpass", "cutoff_hz": 200.0, "slope_db_per_oct": 24}]},
            ),
            EffectSpec(
                "chorus",
                {
    ...
    def _add_chord(
        *,
        ...
        score.add_note(
            ...
            freq=bass_f, amp=0.25, velocity=velocity,
        )
        score.add_note(
            "tenor", start=t_tenor,
            duration=max(0.5, dur_secs - tenor_stagger * spb - TENOR_TRIM),
            freq=tenor_f, amp=0.21, velocity=velocity,
        )
        if not skip_alto:
            score.add_note(
                "alto", start=t_alto,
                duration=max(0.5, dur_secs - alto_stagger * spb),
                freq=alto_f, amp=0.18, velocity=velocity,
            )
        if extra_alto_f is not None:
            score.add_note(
                "alto", start=t_alto,
                duration=max(0.5, dur_secs - alto_stagger * spb),
                freq=extra_alto_f, amp=extra_alto_amp, velocity=velocity,
            )
    ...
    def _add_counter(
        ...
        for freq, dur_beats, velocity in notes:
            ...
            score.add_note(
                "counter", start=t, duration=dur_secs,
                freq=freq, amp=0.22, velocity=velocity,
            )
            t += dur_secs
    ...
    def _add_lead_phrase(
        *,
        ...
        phrase = grid_line(
            freqs, durs, timeline=TL, pitch_kind="freq", amp_db=amp_db
        )
    ...
    score.add_note("bass", start=TL.at(bar=1), duration=TL.measures(2.5),
                   freq=Fs2, amp=0.17, velocity=0.72)
    score.add_note("bass", start=TL.at(bar=3, beat=2), duration=TL.measures(1.5),
                   freq=Fs2, amp=0.20, velocity=0.78)

    # Tenor: sparse arpeggiation over bars 2–4, building a quiet A-major
    # triad before the A section arrives.
    score.add_note("tenor", start=TL.at(bar=2),          duration=q,           freq=A3,  amp=0.10, velocity=0.70)
    score.add_note("tenor", start=TL.at(bar=2, beat=2),  duration=h,           freq=Cs4, amp=0.11, velocity=0.73)
    score.add_note("tenor", start=TL.at(bar=3),          duration=q * 1.5,     freq=B3,  amp=0.10, velocity=0.70)
    score.add_note("tenor", start=TL.at(bar=3, beat=2),  duration=h,           freq=A3,  amp=0.11, velocity=0.72)
    score.add_note("tenor", start=TL.at(bar=4),          duration=TL.measures(1.0), freq=Cs4, amp=0.12, velocity=0.75)
    ...
    for (cbar, cbeat, cdur, bf, tf, af, sus_alto), vel in zip(a_chords, a_velocities, strict=True):
        ...
        if sus_alto is not None:
            ...
            score.add_note(
                "alto", start=TL.at(bar=cbar, beat=cbeat),
                duration=q + 0.15, freq=sus_alto, amp=0.14, velocity=vel * 0.78,
            )
        _add_chord(
            bar=cbar, beat=cbeat, dur_bars=cdur,
            bass_f=bf, tenor_f=tf, alto_f=af,
            velocity=vel,
            alto_stagger=alto_stag, tenor_stagger=1.0,
        )
    ...
    _s = TL.duration(S)
    _add_chord(
        bar=19, beat=0.0, dur_bars=3.0, bass_f=A2, tenor_f=E3, alto_f=Cs4,
        velocity=1.05, skip_alto=True,
    )
    score.add_note("alto", start=TL.at(bar=19),      duration=_s,                     freq=A3,  amp=0.11, velocity=0.88)
    score.add_note("alto", start=TL.at(bar=19) + _s, duration=TL.measures(3.0) - _s,  freq=Cs4, amp=0.18, velocity=1.05)
    ...
    _add_counter(21, 0.0, [(E4, 1.0, 0.90), (B3, 1.0, 0.85), (Fs4, 2.0, 0.88)])  # imitates bar 20
    _add_counter(22, 0.0, [(Fs4, 1.0, 0.92), (Cs4, 1.0, 0.88), (B3, 2.0, 0.85)]) # imitates bar 21
    ...
    _add_chord(
        bar=27, beat=0.0, dur_bars=3.0, bass_f=Fs2, tenor_f=A3, alto_f=Cs4,
        velocity=1.02, bass_stagger=-0.5, alto_stagger=0.0, tenor_stagger=0.5,
    )
    ...
    score.add_note("bass", start=TL.at(bar=30), duration=q + 0.2,
                   freq=Fs2, amp=0.22, velocity=1.05)
    _add_chord(
        bar=30, beat=0.0, dur_bars=2.5, bass_f=B2, tenor_f=D3, alto_f=Fs3,
        velocity=1.08, bass_stagger=1.0, alto_stagger=0.0, tenor_stagger=0.5,
    )

    # V dominant — bass anticipates again for urgency into the cadence.
    _add_chord(
        bar=32, beat=2.0, dur_bars=3.5, bass_f=E3, tenor_f=Gs3, alto_f=B3,
        velocity=1.14, bass_stagger=-0.5, alto_stagger=0.0, tenor_stagger=0.5,
    )

    # Development counter: more exploratory, wider intervals.
    _add_counter(27, 0.0, [(E4, 2.0, 1.05), (Cs4, 2.0, 0.92), (E4, 4.0, 1.00), (D4, 4.0, 0.88)])
    _add_counter(30, 0.0, [(Fs4, 3.0, 1.10), (E4, 2.5, 1.00), (D4, 2.5, 0.90), (Cs4, 2.0, 0.82)])
    _add_counter(32, 2.0, [(Gs4, 3.5, 1.18), (Fs4, 2.5, 1.02), (E4, 4.5, 0.90), (D4, 3.5, 0.80)])
    ...
    _add_chord(
        bar=36, beat=0.0, dur_bars=2.0, bass_f=Fs3, tenor_f=A3, alto_f=Cs4,
        velocity=1.00, alto_stagger=0.5, tenor_stagger=0.5, skip_alto=True,
    )
    score.add_note("alto", start=TL.at(bar=36),        duration=_s,                    freq=Fs3, amp=0.10, velocity=0.82)
    score.add_note("alto", start=TL.at(bar=36) + _s,   duration=_s,                    freq=A3,  amp=0.13, velocity=0.90)
    score.add_note("alto", start=TL.at(bar=36) + 2*_s, duration=TL.measures(2.0)-2*_s, freq=Cs4, amp=0.18, velocity=1.00)
    ...
    for (cbar, cbeat, cdur, bf, tf, af, af2), vel in zip(rep_chords_tail, rep_velocities_tail, strict=True):
        _add_chord(
            bar=cbar, beat=cbeat, dur_bars=cdur,
            bass_f=bf, tenor_f=tf, alto_f=af,
            velocity=vel,
            alto_stagger=0.25, tenor_stagger=0.5,
            extra_alto_f=af2,
        )
    ...
    # Wide vi
    _add_chord(
        bar=44, beat=0.0, dur_bars=3.0,
        bass_f=Fs2, tenor_f=A3, alto_f=Cs5,
        velocity=1.05,
        alto_stagger=1.0, tenor_stagger=1.5,
    )

    # Dm7 — two notes in alto voiced wide
    _add_chord(
        bar=47, beat=0.0, dur_bars=4.0,
        bass_f=D3, tenor_f=F3, alto_f=A3,
        velocity=1.00,
        alto_stagger=1.0, tenor_stagger=1.5,
    )
    ...
    score.add_note(
        ...
        freq=C4, amp=0.16, velocity=1.00,
    )

    # Amaj7
    _add_chord(
        bar=51, beat=0.0, dur_bars=4.0,
        bass_f=A2, tenor_f=E3, alto_f=Cs4,
        velocity=0.88,
        alto_stagger=1.0, tenor_stagger=1.5,
        extra_alto_f=Gs4, extra_alto_amp=0.14,
    )

    # Ending counter: long, slowly fading melodic phrases.
    _add_counter(44, 0.0, [(E4,  4.0, 1.00), (Cs4, 4.0, 0.90), (E4,  4.0, 0.82)])
    _add_counter(47, 0.0, [(F4,  4.0, 1.08), (E4,  4.0, 0.95), (D4,  4.0, 0.85), (C4,  4.0, 0.75)])
    ...
    lead_a: list[tuple[float, Any]] = [
        ...
        (D5, E), (A5, E), (Cs5, Q), (B4, E), (A4, E), (A4, Q),
        # bar 17
        (A4, H), (B4, Q), (A4, Q),
    ...
    lead_a_velocities: list[float] = [
        0.82, 1.00, 0.92,          # bar 4
        0.88, 1.10, 0.95,          # bar 5
        0.95, 0.82, 0.90,          # bar 6
        1.15, 0.90,                # bar 7
        0.88, 1.05,                # bar 8
        0.92, 0.80, 0.85, 1.00,   # bar 9
        1.18, 0.98, 0.85,          # bar 10
        1.05, 0.90, 0.82,          # bar 11
        0.85, 0.95, 1.08,          # bar 12 — held back; Cs5 less emphatic
        0.88, 0.78, 0.85,          # bar 13 — quieter circle (no Cs5)
        0.88, 1.00, 1.10,          # bar 14 — starting to build
        0.92, 0.98, 1.25,          # bar 15 — Cs5 now emphatic; last step before leap
        1.25, 1.42, 1.15, 1.00, 0.88, 0.95,  # bar 16 — D5→A5 apex, descent
        1.05, 0.92, 0.85,          # bar 17
        0.95, 1.05, 0.80,          # bar 18
    ]
    ...
    lead_b_dev_velocities: list[float] = [
        1.00, 1.20,                # bar 19
        0.95, 0.82, 1.05,          # bar 20
        1.05, 0.90, 0.88,          # bar 21
        0.85, 1.10,                # bar 22
        1.05, 0.90, 0.85,          # bar 23
        1.15, 0.95, 0.80,          # bar 24
        0.92, 1.00,                # bar 25
        1.05, 0.88,                # bar 26
        0.90, 0.82, 1.00,          # bar 27
        1.05, 0.92, 0.85,          # bar 28
        0.80, 0.85,                # bar 29
        1.00, 0.88, 0.95,          # bar 30
        0.95, 0.80,                # bar 31
        0.88, 1.05,                # bar 32
        1.25, 1.05,                # bar 33 — Gs4 arrival
        0.92, 1.10, 1.20,          # bar 34
        1.10, 0.90,                # bar 35
    ]
    ...
    lead_rep_end_velocities: list[float] = [
        1.10, 0.95,                # bar 36
        0.88, 0.78, 0.82, 1.05,   # bar 37
        0.85, 1.05,                # bar 38
        1.15, 1.00, 0.90,          # bar 39
        1.05, 0.92,                # bar 40
        0.82, 0.75,                # bar 41
        1.00, 1.10,                # bar 42
        0.95,                      # bar 43
        ...
        0.85,                      # bar 46
        1.00, 1.08,                # bar 47
        0.90, 0.95, 1.05,          # bar 48
        1.10, 1.00,                # bar 49
        0.90,                      # bar 50
        0.95, 0.82,                # bar 51
        0.88, 0.92, 1.00,          # bar 52
        0.95, 0.80,                # bar 53
    ...
    score.add_note("bass", start=TL.at(bar=55), duration=TL.measures(3.0),
                   freq=A2, amp=0.08, velocity=0.52)

    # Counter: a single falling phrase into the silence.
    _add_counter(55, 0.0, [(E4, 2.0, 0.65), (D4, 1.0, 0.58), (Cs4, 1.5, 0.50), (A3, 1.5, 0.42)])

    # Lead: three isolated notes with widening silences; the last is barely audible.
    score.add_note("lead", start=TL.at(bar=55),           duration=TL.duration(H),         freq=Cs5, amp_db=-24.0, velocity=0.62)
    score.add_note("lead", start=TL.at(bar=56),           duration=TL.duration(dotted(H)), freq=A4,  amp_db=-25.5, velocity=0.55)
    score.add_note("lead", start=TL.at(bar=57, beat=2),   duration=TL.duration(W),          freq=E4,  amp_db=-27.0, velocity=0.48)
    score.add_note("lead", start=TL.at(bar=59),           duration=TL.duration(W),          freq=A4,  amp_db=-29.0, velocity=0.40)
...
PIECES: dict[str, PieceDefinition] = {
    "ji_chorale_v2": PieceDefinition(
        ...
        sections=(
            PieceSection(label="Prologue",    start_seconds=TL.at(bar=1),  end_seconds=TL.at(bar=5)),
            PieceSection(label="A",           start_seconds=TL.at(bar=5),  end_seconds=TL.at(bar=19)),
            PieceSection(label="B",           start_seconds=TL.at(bar=19), end_seconds=TL.at(bar=27)),
            PieceSection(label="Development", start_seconds=TL.at(bar=27), end_seconds=TL.at(bar=36)),
            PieceSection(label="Reprise",     start_seconds=TL.at(bar=36), end_seconds=TL.at(bar=44)),
            PieceSection(label="Ending",      start_seconds=TL.at(bar=44), end_seconds=TL.at(bar=55)),
            PieceSection(label="Coda",        start_seconds=TL.at(bar=55), end_seconds=TL.at(bar=60) + 2.0),
        ),
    )
}
